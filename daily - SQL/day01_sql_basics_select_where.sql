...
SELECT name,
       code,
       CONCAT(name, '$', code, 'kushagra') AS combined_string
FROM country;

-- CONCAT_WS function
SELECT name,
       code,
       CONCAT_WS('$', name, code, 'kushagra') AS combined_ws
FROM country;
...
-- Trim spaces
SELECT CHAR_LENGTH(TRIM('        kushagra        ')) AS trimmed_length;

-- Right trim
SELECT CHAR_LENGTH(RTRIM('       kushagra        ')) AS right_trim;

-- Trim specific character
SELECT TRIM(BOTH 'k' FROM 'kkushagra') AS trimmed_word;
